...
// ── InviteEmailModal ──────────────────────────────────────────────────────────
function InviteEmailModal({ open, onClose, trip, activities }) {
  const { t } = useI18n();
  ...
  const handleSend = () => {
    ...
    const subj = encodeURIComponent(`\u2708\uFE0F Invitation voyage : ${tripTitle}`);
    const bodyLines = [
      `Bonjour,`, ``,
      `Tu es invit\u00e9(e) \u00e0 rejoindre le voyage "${tripTitle}" !`, ``,
      `\uD83D\uDCC5 Dates : ${dateRange}`,
      programme ? `\n\uD83D\uDCCB Programme :\n${programme}` : "",
      ``, `\uD83D\uDD17 Rejoins le voyage et t\u00e9l\u00e9charge l'app :`, window.location.origin, ``, `\u00c0 bient\u00f4t ! \uD83C\uDF0D`,
    ].join("\n");
...
// ── ShareModal ────────────────────────────────────────────────────────────────
function ShareModal({ open, onClose, trip, activities }) {
  const { t } = useI18n();
