...
test("chat and vote mutation policies are bound to the authenticated identity", () => {
  ...
  assert.doesNotMatch(sql, /chat_messages_trip_member_insert[\s\S]*WITH\s+CHECK\s*\(\s*public\.trip_id_visible_to_requester\s*\(\s*trip_id\s*\)\s*\)/i);
  assert.doesNotMatch(sql, /activity_votes_trip_member_insert[\s\S]*WITH\s+CHECK\s*\(\s*public\.trip_id_visible_to_requester\s*\(\s*trip_id\s*\)\s*\)/i);
});

test("new Tricount expenses persist explicit split lists instead of a dynamic all-participants sentinel", () => {
  ...
  assert.match(app, /invited_joined_emails:\s*\[\]/);

  const balances = computeTricountBalances(
    ["Moi", "alice@example.com"],
    [{ amount: 90, paid_by: "Moi", split_between: ["Moi"] }]
  );
  assert.equal(balances["Moi"], 90 - 90);
