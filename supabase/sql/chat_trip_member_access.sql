...
BEGIN
  ...
  RETURN EXISTS (
    SELECT 1
    FROM public.trips t
    WHERE t.id::text = p_trip_id::text
      AND (
...
DECLARE
  u uuid := auth.uid();
  em text;
  row_user_id text := trim(COALESCE(p_user_id, ''));
  row_email text := lower(trim(COALESCE(p_email, '')));
BEGIN
  IF u IS NULL THEN
    RETURN false;
  END IF;
  ...
  IF em IS NULL OR em = '' THEN
    RETURN false;
  END IF;
  IF row_user_id = '' AND row_email = '' THEN
    RETURN false;
  END IF;
  IF row_user_id <> '' AND row_user_id <> u::text THEN
    RETURN false;
  END IF;
  IF row_email <> '' AND row_email <> em THEN
    RETURN false;
  END IF;
  RETURN true;
END;
