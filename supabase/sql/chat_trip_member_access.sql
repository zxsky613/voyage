...
DECLARE
  ...
  em text;
BEGIN
  IF u IS NULL OR NULLIF(trim(COALESCE(p_user_id, '')), '') IS NULL THEN
    RETURN false;
  END IF;
  IF trim(p_user_id) <> u::text THEN
    RETURN false;
  END IF;
  SELECT lower(trim(au.email::text)) INTO em FROM auth.users au WHERE au.id = u;
  IF em IS NULL OR em = '' OR NULLIF(trim(COALESCE(p_email, '')), '') IS NULL THEN
    RETURN false;
  END IF;
  RETURN lower(trim(p_email)) = em;
END;
$$;
