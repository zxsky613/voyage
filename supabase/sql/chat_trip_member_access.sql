...
CREATE OR REPLACE FUNCTION public.requester_normalized_email()
RETURNS text
LANGUAGE sql
STABLE
SECURITY DEFINER
...
REVOKE ALL ON FUNCTION public.requester_normalized_email() FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.requester_normalized_email() TO authenticated;
GRANT EXECUTE ON FUNCTION public.requester_normalized_email() TO service_role;
...
CREATE POLICY "chat_messages_trip_member_insert" ON public.chat_messages
  ...
  WITH CHECK (
    ...
    AND lower(trim(author_email)) = public.requester_normalized_email()
  );
...
CREATE POLICY "activity_votes_trip_member_insert" ON public.activity_votes
  ...
  WITH CHECK (
    ...
    AND lower(trim(voter_email)) = public.requester_normalized_email()
  );
