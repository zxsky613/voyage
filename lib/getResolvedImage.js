...
async function fetchResolvedImage(params) {
  try {
    ...
    return {
      ...
      source: j.source,
      heroSource: j.heroSource || j.source || "fallback",
...
/**
 * Commons geosearch via /api/images/resolve (preferGeo + coords).
 * @param {{ latitude: number, longitude: number, label: string, uiLang?: string }} params
 */
export async function getResolvedGeoActivityImage(params) {
  const lat = Number(params.latitude);
  const lon = Number(params.longitude);
  const label = String(params.label || "").trim();
  if (!Number.isFinite(lat) || !Number.isFinite(lon) || !label) return null;

  const key = `geo|${lat}|${lon}|${label}|${params.uiLang || "fr"}`;
  if (memo[key]) return memo[key];

  const task = runActivityResolveQueued(async () => {
    try {
      const r = await fetch(getImagesApiPostUrl("resolve"), {
        method: "POST",
        headers: { "Content-Type": "application/json" },
      ...
      return {
        url: String(j.url).trim(),
        source: j.source,
        photoSource: j.photoSource || "wikimedia_geo",
        heroSource: j.heroSource || "fallback",
        attribution: j.attribution,
        entityId: j.entityId,
        cached: j.cached,
      };
