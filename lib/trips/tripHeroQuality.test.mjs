...
test("malformed percent-encoding must not throw (render-safe)", () => {
  const poisoned = [
  ...
  for (const url of poisoned) {
    assert.doesNotThrow(() => isBlockedTripHeroUrl(url));
    assert.doesNotThrow(() => isValidPersistedTripHeroUrl(url));
    assert.doesNotThrow(() => readTripHeroUrlFromTrip({ hero_image_url: url }));
    assert.equal(isValidPersistedTripHeroUrl(url), false);
    assert.equal(readTripHeroUrlFromTrip({ hero_image_url: url }), "");
  }
});
