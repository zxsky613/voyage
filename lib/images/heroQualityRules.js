...
import { safeDecodeURIComponent } from "./commonsThumbUrl.js";
...
export function extractCommonsFileTitleFromUrl(url) {
  const u = safeDecodeURIComponent(String(url || ""));
...
export function scoreHeroSuspicion(input = {}) {
  ...
  const hay = `${fileTitle} ${safeDecodeURIComponent(url)} ${input.categories || ""}`.replace(/_/g, " ").toLowerCase();
