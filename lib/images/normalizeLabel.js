...
/** Synonymes pays/région → forme anglaise stable pour clés cache (grece → greece). */
const GEO_CONTEXT_ALIASES = Object.freeze({
  grece: "greece",
  grce: "greece",
  hellas: "greece",
  espagne: "spain",
...
export function normalizeGeoContextKey(context) {
  const raw = normalizeKeyPart(context);
  if (!raw) return "";
  return raw
    .split(",")
    .map((part) => {
      const p = part.trim();
      return GEO_CONTEXT_ALIASES[p] || p;
    })
    .join(", ");
}
...
export function normalizeLabel(label, context = "") {
  ...
  if (!base) return "";
  const ctxNorm = normalizeGeoContextKey(geoContext);
