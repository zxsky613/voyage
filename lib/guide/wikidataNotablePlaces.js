...
/** Types P31/P279 acceptés — lieux visitables (pas de « building » générique). */
export const NOTABLE_TYPE_QIDS = [
...
/** P31 à exclure (stade, université, hôpital, gare, ministère, club sportif…). */
export const EXCLUDED_MUST_SEE_TYPE_QIDS = [
  "Q483110", // stadium
  "Q875538", // university
  "Q3914", // school
  ...
  "Q262166", // government ministry
  "Q327333", // government agency
  "Q847017", // sports club
  "Q476028", // association football club
  "Q15944511", // multi-sport club
];

const TTL_MS = 24 * 60 * 60 * 1000;
/** Bump pour invalider le cache in-memory post-déploiement (filtre P31). */
const WIKIDATA_NOTABLE_CACHE_SCHEMA = "p31-v1";
...
export function wikidataNotableCacheKey(lat, lon, locale, radiusKm = 8) {
  return `${WIKIDATA_NOTABLE_CACHE_SCHEMA}|${Number(lat).toFixed(3)}|${Number(lon).toFixed(3)}|${String(locale || "fr").slice(0, 2)}|${radiusKm}`;
}

/** Purge ciblée (coords arrondies 3 décimales) — recette post-fix P31. */
export function purgeWikidataNotableCacheNear(lat, lon) {
  const needle = `${Number(lat).toFixed(3)}|${Number(lon).toFixed(3)}`;
...
function buildSparqlQuery(lat, lon, uiLang, radiusKm, minSitelinks, limit) {
  ...
  const excludedValues = EXCLUDED_MUST_SEE_TYPE_QIDS.map((q) => `wd:${q}`).join(" ");
  return `
SELECT ?placeLabel ?sitelinks WHERE {
  ...
  FILTER(NOT EXISTS {
    ?place wdt:P31/wdt:P279* ?x .
    VALUES ?x { ${excludedValues} }
  })
