...
test("mergeMustSeePlaceCandidates prioritizes iconic over FSQ/OSM noise", () => {
  const merged = mergeMustSeePlaceCandidates({
    iconicNames: ["Gorges de Samaria", "La Canée"],
    highlightsNames: [],
    osmNames: ["Knossos", "Museo Civico"],
    fsqNames: ["Museo delle Marionette"],
  });
  assert.equal(merged[0], "Gorges de Samaria");
  assert.ok(merged.includes("Knossos"));
  assert.ok(merged.indexOf("Gorges de Samaria") < merged.indexOf("Museo delle Marionette"));
});
