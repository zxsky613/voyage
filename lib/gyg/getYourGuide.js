...
/** Teaser desktop : une rangée ~3 offres (guide). */
export const GYG_WIDGET_DESKTOP_MAX_HEIGHT_PX = 400;
export const GYG_WIDGET_NUMBER_OF_ITEMS = 4;
export const GYG_WIDGET_DESKTOP_NUMBER_OF_ITEMS = 3;
...
/** @param {{ cityLabel: string, language: string, partnerId: string, numberOfItems?: number, desktop?: boolean }} input */
export function buildGygWidgetMountConfig(input = {}) {
  ...
  const lid = resolveGetYourGuideLocationId(cityToken);
  const defaultCount = input.desktop ? GYG_WIDGET_DESKTOP_NUMBER_OF_ITEMS : GYG_WIDGET_NUMBER_OF_ITEMS;
  const itemCount = Math.min(4, Math.max(3, Number(input.numberOfItems) || defaultCount));
  const dataset = {
    ...
    "data-gyg-number-of-items": String(itemCount),
