...
export function buildVeventBlock(activity, opts = {}) {
  ...
  const dtEnd = addHoursToIcsLocalDateTime(dtStart, hours) || dtStart;
 ...
 * @returns {string}
 */
export function buildTripIcs({ destination = "", activities = [], appUrl = DEFAULT_APP_URL } = {}) {
  const dest = String(destination || "").trim();
  const events = (Array.isArray(activities) ? activities : [])
    .map((activity, index) =>
      buildVeventBlock(activity, { destination: dest, appUrl, index })
    )
    .filter(Boolean);
