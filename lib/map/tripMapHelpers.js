 ...
 * @param {{ padding?: number, animate?: boolean, maxZoom?: number, uniformPadding?: boolean }} [opts]
 */
export function fitMapToActivities(map, activities, opts = {}) {
  ...
  const duration = opts.animate === false ? 0 : undefined;
  if (pts.length === 1) {
    map.easeTo({
...
/**
 * Cadrage vue d'ensemble : marqueurs-jour spiderfiés + ancres des traits leader.
 * @param {import('maplibre-gl').Map} map
 * @param {{ markers?: { features?: object[] }, leaderLines?: { features?: object[] } }} layout
 * @param {Array<{ latitude: number, longitude: number }>} [centroids]
 * @param {{ padding?: number, animate?: boolean, maxZoom?: number }} [opts]
 */
export function fitMapToTripDayLayout(map, layout, centroids, opts = {}) {
  /** @type {number[][]} */
  const coords = [];
  ...
  for (const f of layout?.markers?.features || []) {
    const c = f?.geometry?.coordinates;
    if (Array.isArray(c) && c.length >= 2) coords.push(c);
  }
  for (const f of layout?.leaderLines?.features || []) {
    const anchor = f?.geometry?.coordinates?.[1];
    if (Array.isArray(anchor) && anchor.length >= 2) coords.push(anchor);
  }
