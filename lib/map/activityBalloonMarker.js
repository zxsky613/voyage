...
export function dayPinImageId(dayIndex, dayNum, selected = false) {
  ...
  return selected ? `day-drop-${d}-${n}-sel` : `day-drop-${d}-${n}`;
}

/**
 * Goutte d'eau vue d'ensemble — pointe basse sur le centroïde, orange brand, numéro blanc.
 * @param {number|string} dayNum
 * @param {string} [_fillColor] ignoré — toujours orange brand (distinction par numéro)
 * @param {{ selected?: boolean }} [opts]
 */
export function buildDayPinSvg(dayNum, _fillColor, opts = {}) {
  const n = String(dayNum || "1");
  const selected = Boolean(opts.selected);
  const fill = ACTIVITY_BALLOON_ORANGE;
  const scale = selected ? 1.12 : 1;
  const w = Math.round(38 * scale);
  const h = Math.round(52 * scale);
  const strokeW = selected ? 3.5 : 3;
  const fontSize = selected ? 15 : 14;

  return `<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}" viewBox="0 0 32 44" role="img" aria-hidden="true">
  <path d="M16 41 C16 41 3.5 25.5 3.5 15.5 C3.5 7.5 9.5 2.5 16 2.5 C22.5 2.5 28.5 7.5 28.5 15.5 C28.5 25.5 16 41 16 41 Z" fill="${fill}" stroke="#FFFFFF" stroke-width="${strokeW}" stroke-linejoin="round"/>
  <text x="16" y="17.5" text-anchor="middle" dominant-baseline="middle" font-family="Arial,Helvetica,sans-serif" font-size="${fontSize}" font-weight="700" fill="${ACTIVITY_BALLOON_NUM}">${n}</text>
</svg>`;
}

/**
 * Gouttes-jour orange (vue d'ensemble) — ancrées par la pointe sur le centroïde.
 * @param {import('maplibre-gl').Map} map
...
export async function registerDayPinImages(map, specs, selectedDayIndex, opts = {}) {
  ...
  for (const spec of specs || []) {
    ...
    for (const selected of [false, true]) {
      ...
      if (map.hasImage(id)) continue;
      jobs.push(
        loadSvgImage(map, id, buildDayPinSvg(dayNum, ACTIVITY_BALLOON_ORANGE, { selected }), timeoutMs)
      );
    }
  }
  if (!jobs.length) return true;
