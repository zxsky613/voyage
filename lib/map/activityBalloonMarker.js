...
/**
 * Pastille ronde numérotée (vue d'ensemble) — ancrée au centroïde, sans tige.
...
export function buildDayPinSvg(dayNum, fillColor, opts = {}) {
  ...
  const r = selected ? 13.5 : 12;
  const strokeW = selected ? 3 : 2.5;
  const fontSize = selected ? 13 : 12;
  return `<svg xmlns="http://www.w3.org/2000/svg" width="${size}" height="${size}" viewBox="0 0 30 30" role="img" aria-hidden="true">
  <circle cx="15" cy="15" r="${r}" fill="${fillColor}" stroke="#FFFFFF" stroke-width="${strokeW}"/>
  <text x="15" y="15.5" text-anchor="middle" dominant-baseline="middle" font-family="Arial,Helvetica,sans-serif" font-size="${fontSize}" font-weight="700" fill="#FFFFFF">${n}</text>
</svg>`;
}

/**
 * Pastilles-jour (couleur jour) pour vue d'ensemble — cercle centré sur le centroïde.
 * @param {import('maplibre-gl').Map} map
...
export async function registerDayPinImages(map, specs, selectedDayIndex, opts = {}) {
  ...
  for (const spec of specs || []) {
    ...
    const color = String(spec.color || ACTIVITY_BALLOON_ORANGE);
    for (const selected of [false, true]) {
      if (selected && dayIndex !== selectedDayIndex) continue;
      const id = dayPinImageId(dayIndex, dayNum, selected);
      if (map.hasImage(id)) continue;
      jobs.push(
        loadSvgImage(map, id, buildDayPinSvg(dayNum, color, { selected }), timeoutMs)
      );
    }
  }
  if (!jobs.length) return true;
