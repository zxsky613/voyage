...
test("sortItineraryDaysChronologically uses the displayed order for later actions", () => {
  const input = [
    {
      day: 1,
      bullets: ["Après-midi : Knossos", "Matin : Elafonisi"],
    },
  ];

  const ordered = sortItineraryDaysChronologically(input);

  assert.deepEqual(ordered[0].bullets, ["Matin : Elafonisi", "Après-midi : Knossos"]);
  assert.deepEqual(input[0].bullets, ["Après-midi : Knossos", "Matin : Elafonisi"]);
});
