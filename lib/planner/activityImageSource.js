...
export function isActivityPhotoPlaceholder(activityOrMeta) {
  const source = String(activityOrMeta?.photoSource || activityOrMeta?.photo_source || "");
  return source === "placeholder" && !pickResolvedActivityPhoto(activityOrMeta);
}
...
export function pickActivityDisplayPhotoUrl(activityOrMeta, storedPhotoUrl = "") {
  return pickResolvedActivityPhoto(activityOrMeta) || String(storedPhotoUrl || "").trim();
}
...
export function buildActivityPhotoFieldsForPersist(meta, modalCachedUrl = "") {
  ...
  if (isActivityPhotoPlaceholder(meta) && !/^https?:\/\//i.test(cached)) {
  ...
  if (!photo_source || photo_source === "none" || (photo_source === "placeholder" && photo_url)) {
    if (photo_url) {
