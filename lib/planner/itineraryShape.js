...
import { sortDayActivitiesChronologically } from "./itineraryDayOrder.js";
...
export function deriveBulletsFromActivities(day, lang = "fr") {
  const acts = sortDayActivitiesChronologically(Array.isArray(day?.activities) ? day.activities : []);
  if (acts.length) {
    return acts
      .map((a) => {
...
export function normalizeVerifiedDayIdeas(dayIdeas, lang = "fr") {
  return applyComputedDayCosts(
    (Array.isArray(dayIdeas) ? dayIdeas : []).map((day) => {
      const activities = sortDayActivitiesChronologically(
        Array.isArray(day?.activities) ? day.activities : []
      );
      const bullets = deriveBulletsFromActivities({ ...day, activities }, lang);
