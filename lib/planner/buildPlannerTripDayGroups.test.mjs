...
test("buildPlannerTripDayGroups drops activities outside trip date range", () => {
  const groups = buildPlannerTripDayGroups({
    tripId: "t1",
    tripStartYmd: "2026-07-24",
    tripEndYmd: "2026-07-26",
    cityLabel: "Cassis",
    activities: [
      { id: "before", trip_id: "t1", date: "2026-07-23", time: "10:00", title: "Orphan before" },
      { id: "in", trip_id: "t1", date: "2026-07-25", time: "10:00", title: "In range", location: "Port" },
      { id: "after", trip_id: "t1", date: "2026-07-27", time: "10:00", title: "Orphan after" },
    ],
  });
  assert.equal(groups.length, 1);
  assert.equal(groups[0].activities.length, 1);
  assert.equal(groups[0].activities[0].id, "in");
});
