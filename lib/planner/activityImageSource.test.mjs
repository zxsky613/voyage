...
test("pickPlannerActivityStoredPhotoUrl reads photo_url despite placeholder source", () => {
  const url = "https://commons.wikimedia.org/wiki/Special:FilePath/Cassis.jpg";
  const activity = { photo_url: url, photo_source: "placeholder" };
  assert.equal(pickPlannerActivityStoredPhotoUrl(activity), url);
  assert.equal(hasPersistedActivityPhotoUrl(activity), true);
});
