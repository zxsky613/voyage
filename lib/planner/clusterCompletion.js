...
/**
 * Complète les jours sous la cible avec les meilleurs candidats non assignés.
 * @param {object[][]} dayAssignments
 * @param {object[]} scored
 * @param {number|number[]} perDayOrTargets
 * @param {number} days
 */
export function contractRelaxFill(dayAssignments, scored, perDayOrTargets, days) {
  ...
  for (let d = 0; d < days; d += 1) {
    const target = Math.max(1, Number(targets[d]) || 1);
    while (out[d].length < target && spare.length) {
